...
- Local: Python 3.13、178 passed / 1 skipped / 2 deprecation warnings。SkipはAzure apply/資格情報を必要とする既存external placeholderで、Azure PASSへ変更していない。Stage A 14 positive/negative、Stage B 6 injection、S1/S5 Healthy、Session/History、MCP fixture、status分離を保持。
- 既知のP2制約: (1) Code子の現行contractは一般化した`NOT_FOUND`も返し得るため、親だけでは「部署未登録」と「勘定科目/分類mapping未登録」を常に判別できず、後者でも部署の再入力を求める場合がある。正しく分離するにはCode Promptのreason-specific contract/version更新が必要。(2) partial intakeを持つSessionへ通常のHosted確認経路を通さず完全な`ProcurementRequest`を直接送る外部callerは、明示的なconfirmationでなくてもfail-closedの`confirmation_state_invalid`となり得る。Web/Hostedの既定経路は最終「確定」時だけ完全requestを作る。いずれもdraftを作らず停止するためmerge blockerとはせず、意味の異なる成功へ変換もしない。
- Hosted v22 選択候補の仕様検証: Reviewで発見したP1に対応し、子Agent推奨候補とは別の商品をユーザーが選んだ場合も、その選択商品のSearch-grounded仕様を要求仕様と照合する。source ZIP SHA256 `8bc17940787ef4193c616993657e5843d389010c2ba886dd7ceaf4fb56880562`をACTIVE確認。Direct Synthetic case `S4-SELECTED-SPEC-V22-20260905013922`、Conversation `conv_0eb44712a13b1d8000gwROFoVZVUCAzg0Xs5OokPD39hpzmYJu`は、turn 1でノートPC2候補を提示し、turn 2で16GBの`LAPTOP-OFFICE-13`を選択しながら32GB要件を追加すると`VALIDATION_FAILED / catalog_specification_mismatch`で入力時に拒否した。確認票・draft・Code子呼出しへ進まない。turn 2 Trace `2fa1e8c4b431ad9b02dfa180b2f5c1fa`はAgent v22、turn 2、同statusを含む31行のallowlist JSONLへexportした。先行した非stream試行は初回turnが240秒でread timeout、stream再試行は完了したため、非stream latencyは未解消。
- Hosted v21 商品変更Session回帰: 最終ReviewのP1に対応し、商品query変更時は選択、Catalog結果、Code結果をFramework AgentSessionへ即時保存して無効化してからControllerへ渡す。候補表示時も商品名、分類、単価、通貨、仕様をSearch evidence snapshotと完全一致させる。source ZIP SHA256 `33193f3b9611be74e6536746b5190f69b0b265a45fde4e2758339a0f558e8793`をACTIVE確認。Direct Synthetic case `CHAT-QUERY-CHANGE-V21-20260905011706`、Conversation `conv_02bbc52b8458f5d100nlDn1tucSXG7mF549wmW8T0oRiNA93FI`は、turn 1でノートPC2候補、turn 2の「名刺を購入したい」でCatalog/Searchを再実行し、business `WAITING_USER`、reason `SINGLE_EXACT_CATALOG_MATCH`、次項目を数量だけとして終了した。旧商品候補、確認票、draftは返さず、Framework Session hashを維持。turn 2 Trace `be1d24c8842053513c3cfbdeb3cfc374`はAgent v21、turn 2、Catalog step SUCCESS、response WAITING_USERを含む46行のallowlist JSONLへexportした。
- Hosted v20 申請者名付き確認票・即時確定: source ZIP SHA256 `16821e53204f3f609645724b3a6a93ae521431597d3b6a42f6d9bda9ac191e8c`をACTIVE確認。Direct Synthetic case `CHAT-NAME-V20-20260905005503`、Conversation `conv_0f6e2b4be3f9a79c00yhlWq95tv9BrwVRubVWTjcF2ZOwuWBb0`は、EasyAuth相当metadataの表示名を確認票へ出し、商品/code/分類/仕様/単価/数量/小計、部署名/code、勘定科目名/code、メモを表示した。5turnは49.282/14.208/13.249/53.530/1.478秒、Framework Session hashは全turn同一。最終「確定」は保存済みCatalog/Code evidenceを`step.reused`し、Prompt Agent/Toolbox/Searchを呼ばず、`merge_validate`だけでSUCCESS。App Insightsの最終Hosted requestは1,201.773msで、Catalog/Code/Tool spanなし。確認票turn 47行・確定turn 23行をallowlist JSONLへexportした。Synthetic名は応答本文へ表示する一方、custom属性での氏名一致0件、`gen_ai.input.messages`/`gen_ai.output.messages`収集0件を確認。Browser EasyAuth実ユーザー名の表示確認ではないため、実ユーザー経路は未検証のまま。
- Latency実測: v20の初回49.282秒と確認票生成53.530秒に対し、最終確定は1.478秒。確認票turnのCode子`invoke_agent`は22.111秒、そこに至る親Responses呼出しも含まれる一方、決定論的mergeは0ms級。以前のv18実測でもMCP Toolbox/Search envelopeは約1.5〜1.7秒で、主要因はServerless Searchではなく親/子LLM推論とmanaged orchestration。Developer APIMのstreamingは進捗の体感を改善するが総処理時間を短縮しない。
- Hosted v18 名刺順次intake: RepositoryへSynthetic商品`BUSINESS-CARD-01`とprinting科目mappingを追加し、source_version `2026-09-04.1`へ更新。既存Blob/Indexerを再利用し、Catalog 11件・Code Master 10件をread-back一致で投入した。Indexerは11/10件処理、失敗0・エラー0・警告0、両Indexの全Repository projection一致。source ZIP SHA256 `78242817149e7e2f4d938e8f0d462f86c4632bb07ee879cb978f58ae21a68f94`の親v18をACTIVE確認。Case `CHAT-NAMECARD-20260904121318`は「名刺を購入したい」からSearch根拠付き完全一致1件を`BUSINESS-CARD-01`へ決定し、数量→所属部署→メモ→確定の順で5turnを完走。最終draftは10個、`DPT-SALES`、memo=`なし`、全turn同一Conversation/Framework Session hash。turn 2/3はCatalogを再実行せず、turn 4でCode事前検証、turn 5 response内Plan eventでCatalog→Code→mergeを最終再実行した。旧Versionで商品未検出後に後続項目を保存したSessionも、同じ購入依頼を明示し直した場合だけ失敗探索と先取り項目を消して商品stepから再開するLocal regressionを追加した。turn 5のCatalog `plan.step.execute` custom spanだけはApp Insightsに記録されず`NOT_RECORDED_BY_PLATFORM`、response eventと業務結果をspan取得済みと見せかけない。Web package/resource/RBAC/費用SKUは変更していない。
- Hosted v16 入力時部署検証: 4項目が揃った時点で未検証入力をローカルにstageし、保存済みCatalog evidenceを再利用してCode Prompt→Toolbox→Searchで所属部署を照合する。検証中の部署値はAgentSessionへ確定保存しない。未登録なら`DEPARTMENT_NOT_FOUND`として確定前に再入力を求め、商品選択・Catalog候補は保持する。照合成功後だけ元の自然言語queryを含むintakeへ保存して「確定」を求め、最終「確定」ではCatalog/Codeを再実行する。source ZIP SHA256 `d6843028fdcd7f69224f5c1ab0ae6f47022ba3a85e4764849be47df81c75f49b`をACTIVE確認。Azure case `CHAT-DIRECT-20260904082913`は2番目の商品選択後、未登録部署をturn 3で32.815秒・outer WAITING_USER/inner NOT_FOUND/missing department/draftなしとして拒否。登録済み部署への訂正はCatalog再実行なしで28.529秒・confirmation待ち、turn 5はCatalog/Codeを最終再取得して32.322秒・全status SUCCESS・draftあり。同一Conversation/Framework Session hashを5turnで維持し、完了後Plan clearもPASS。Web package/resource/RBACは変更していない。
- Hosted v13 メモなし対応: メモ不足時の公開応答を「メモ（ない場合は『なし』）」へ変更。ユーザーが「なし」「特になし」等を明示した場合だけ`memo=なし`へ正規化し、単なる省略からは推測しない。source ZIP SHA256 `2dd8862de18560a18bfdbb36303f16fde2d1affb453e2316c0d3339d8fc514bc`をACTIVE確認。Azure case `CHAT-DIRECT-20260904053301`は「メモはなしです」を含む4turnを39.374/1.372/12.167/57.652秒で完走し、確定draftのmemoが`なし`であることをscript assertionで確認。全status SUCCESS、同一Conversation/Framework Session hash。Web package変更はなく、Webからagent名endpointが参照するactive Hosted v13へ反映される。
- 最終Review P1対応（Hosted v12）: 新規購買intentを固定5語だけでなく「欲しい」「注文」「発注」「買う」と数量表現へ拡張。一般会話/本人確認turnも`last_machine_response`へSUCCESS/NOT_RUNを保存し、turn番号付き`response.generate` spanを生成する。Local normalizeは同一`test.case.id`を複数turnで再利用しても現在turnだけを選ぶ。v12 source ZIP SHA256 `75710ad68ed77b0f65fb61a3fb2f93228d3c0ea528d709695ceedeb20b3d9f5a`をACTIVE確認。Azure case `CHAT-DIRECT-20260904051353`は「ノートPCが欲しい」からCatalogへ入り、37.912/1.488/9.677/41.046秒で確定まで成功。同一Framework Session hashを維持し、完了後の「確定」はCHAT・MCP/Search NOT_RUN、Playground挨拶/本人確認は非JSON自然言語。Reviewの3件を解消した。
- Hosted v11 / Web最新配備: 完了済みPlanを次turn開始時に初期化し、「確定」の再入力を購買再実行にしない。保存中の購買Planがある場合だけ継続し、新規turnは購買語を含む入力のみPlan & Executeへ進める。「こんにちは」は一般会話、「私は誰？」は本人確認へ分岐する。Webは機械契約`app.client.contract=web-json-v1`を明示し、Playground/通常ResponsesはJSONではなく自然言語を返す。Hosted source ZIP v11 SHA256 `b2d95668945a1df8f991bb053a8913b71560f28fe22e3ea615bcf6399b86c2c0`とWeb ZIP SHA256 `b2fccfae7b5965ec61e1d5e8faeedc315ff7185cc25516ee171bc27ab2d037b6`を配備。Web deploymentは`e857af08-219a-437c-8d59-572a6fd86fd2`、匿名401を維持。
- Azure direct case `CHAT-DIRECT-20260904045406`: 同一Conversation `conv_00570230023f3b3000IiAkiv2dOGhw61QIeb7VJTUK1kP3VTEP` / Framework Session hashで候補提示→選択→不足情報→確定を44.159/1.941/10.984/59.802秒で完走。turn 2/3はCatalogを再実行せず、確定は全status SUCCESS・draftあり。直後の5回目「確定」は`CHAT/general`、MCP/Search `NOT_RUN`となり、完了Planのclearを実測した。別Conversationで「こんにちは」「私は誰？」がJSONではなく自然言語応答になることも確認。PlaygroundにはEasyAuth claimが渡らないため、本人名の実取得には別構成のOAuth Identity Passthrough MCP＋Graph OBOが必要であり、現時点では説明応答まで。Webでは検証済みEasyAuth表示名を申請者へ使い、Trace属性/baggageには保存しない。
- Computer Use実画面検証は、Windows helperがWSL workspace URIを受理せず`codex/sandbox-state-meta: sandboxCwd is not a local file URI`で初期化不能。Azure direct、Web package/deployment、EasyAuth匿名401、既存Browser成功Traceとは区別し、今回のv11挨拶/本人確認をBrowser PASSにはしない。
...
- 最終Review P1対応: Catalog候補のcode/evidence IDだけでなく、商品名、分類、単価、通貨、仕様をEvidence snapshotと一致させるboundary validationを追加。各field改変5件をS4/BLOCKEDへする回帰を追加した。Catalog Prompt v4と、それを参照するHosted v10 source ZIP SHA256 `e0432937709f6a453072e5a4254bdd098b384eb45e7ef98dfc59560c793c7d9f`はactive/enabled、identity active。case `CHAT-DIRECT-20260904043500`は同一Conversation/Framework Session hashで4 turnを56.373/1.704/20.373/58.852秒で完走し、確定turn Trace `42ab71559c70d3408e45dd7ccb5cd383`で全status SUCCESS・draftあり。P1を解消した。ReviewのP2（Code表示名のEvidence照合、30日EasyAuth credential自動rotation）はmerge blockerではなく未対応。
