...
- 既知のP2制約: Code子の現行contractは一般化した`NOT_FOUND`も返し得るため、親だけでは「部署未登録」と「勘定科目/分類mapping未登録」を常に判別できず、後者でも部署の再入力を求める場合がある。正しく分離するにはCode Promptのreason-specific contract/version更新が必要。draftを作らず停止するためmerge blockerとはせず、意味の異なる成功へ変換もしない。
